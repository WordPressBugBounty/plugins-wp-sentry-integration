...
final class RuntimeContextManager
{
    ...
    public function __construct(\Sentry\State\HubInterface $baseHub)
    {
     ...
     */
    public function setCurrentHub(\Sentry\State\HubInterface $hub) : bool
    {
    ...
    public function getCurrentHub() : \Sentry\State\HubInterface
    {
        return $this->getCurrentContext()->getHub();
    }
    public function getCurrentContext() : \Sentry\State\RuntimeContext
    {
        $executionContextKey = $this->getExecutionContextKey();
        if ($this->hasActiveContextForExecutionContextKey($executionContextKey)) {
            $runtimeContextId = $this->executionContextToRuntimeContext[$executionContextKey];
    ...
    private function createContextForExecutionContextKey(string $executionContextKey) : void
    {
        ...
        $runtimeContext = new \Sentry\State\RuntimeContext($runtimeContextId, $this->createHubFromBaseHub());
    ...
    private function flushRuntimeContextResources(\Sentry\State\RuntimeContext $runtimeContext, ?int $timeout, \WPSentry\ScopedVendor\Psr\Log\LoggerInterface $logger) : void
    {
        ...
        try {
            ...
            $logger->error('Failed to flush logs while ending a runtime context.', ['exception' => $exception, 'runtime_context_id' => $runtimeContext->getId()]);
        }
        ...
        try {
            ...
            $logger->error('Failed to flush trace metrics while ending a runtime context.', ['exception' => $exception, 'runtime_context_id' => $runtimeContext->getId()]);
        }
    ...
    private function createHubFromBaseHub() : \Sentry\State\HubInterface
    {
        if (!$this->baseHub instanceof \Sentry\State\Hub) {
            return new \Sentry\State\Hub($this->baseHub->getClient());
        }
        $clonedScope = null;
        $this->baseHub->configureScope(static function (\Sentry\State\Scope $scope) use(&$clonedScope) : void {
            $clonedScope = clone $scope;
            // Do not inherit active traces into a new runtime context.
            $clonedScope->setSpan(null);
            $clonedScope->setPropagationContext(\Sentry\Tracing\PropagationContext::fromDefaults());
        });
        return new \Sentry\State\Hub($this->baseHub->getClient(), $clonedScope ?? new \Sentry\State\Scope());
    }
    private function getLoggerFromHub(\Sentry\State\HubInterface $hub) : \WPSentry\ScopedVendor\Psr\Log\LoggerInterface
    {
        $client = $hub->getClient();
        if ($client === null) {
    ...
    private function getGlobalContext() : \Sentry\State\RuntimeContext
    {
        if ($this->globalContext === null) {
            // Lazy fallback keeps baseline behavior when users do not opt into explicit context lifecycle.
            $this->globalContext = new \Sentry\State\RuntimeContext('global', $this->baseHub);
        }
