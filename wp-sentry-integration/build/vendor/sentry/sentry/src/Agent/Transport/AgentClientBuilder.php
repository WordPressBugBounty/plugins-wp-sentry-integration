...
final class AgentClientBuilder
{
    ...
    public function setFallbackClient(\Sentry\HttpClient\HttpClientInterface $fallbackClient) : self
    {
    ...
    public function getClient() : \Sentry\Agent\Transport\AgentClient
    {
        if ($this->isFallbackClientDisabled) {
            return new \Sentry\Agent\Transport\AgentClient($this->host, $this->port, null);
        }
        if ($this->fallbackClientFactory !== null) {
            return new \Sentry\Agent\Transport\AgentClient($this->host, $this->port, $this->fallbackClientFactory);
        }
        return new \Sentry\Agent\Transport\AgentClient($this->host, $this->port, $this->createDefaultFallbackClientFactory());
    }
    /**
    ...
    private function createDefaultFallbackClientFactory() : callable
    {
        ...
        return static function () use($sdkIdentifier, $sdkVersion) : HttpClientInterface {
            return new \Sentry\HttpClient\HttpClient($sdkIdentifier, $sdkVersion);
        };
    }
}
