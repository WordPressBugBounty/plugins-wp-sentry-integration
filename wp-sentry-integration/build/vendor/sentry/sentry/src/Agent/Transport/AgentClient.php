...
use Sentry\Options;
class AgentClient implements \Sentry\HttpClient\HttpClientInterface
{
    ...
    private function writePayload(string $payload) : bool
    {
        ...
        $totalWrittenBytes = 0;
        $writeDeadline = \microtime(\true) + self::SOCKET_TIMEOUT_SECONDS;
    ...
    {
        $remainingSeconds = $deadline - \microtime(\true);
    ...
    private function getFallbackClient() : ?\Sentry\HttpClient\HttpClientInterface
    {
        ...
        if (!$fallbackClient instanceof \Sentry\HttpClient\HttpClientInterface) {
            $this->fallbackClientFactory = null;
    ...
    public function sendRequest(\Sentry\HttpClient\Request $request, \Sentry\Options $options) : \Sentry\HttpClient\Response
    {
        ...
        $logContext = ['agent_host' => $this->host, 'agent_port' => $this->port];
        ...
        if ($fallbackClient !== null) {
            ...
            try {
                ...
                return new \Sentry\HttpClient\Response(502, [], \sprintf('Failed to send envelope using fallback HTTP client. Reason: "%s".', $exception->getMessage()));
            }
        }
        ...
        return new \Sentry\HttpClient\Response(502, [], 'Failed to send envelope to the local Sentry agent and no fallback client is available.');
    }
}
