...
class ClientReportAggregator
{
     ...
     */
    private $reports = [];
    public function add(\Sentry\Transport\DataCategory $category, \Sentry\ClientReport\Reason $reason, int $quantity) : void
    {
    ...
    public function flush() : void
    {
        ...
        foreach ($this->reports as $category => $reasons) {
            foreach ($reasons as $reason => $quantity) {
                $reports[] = new \Sentry\ClientReport\DiscardedEvent($category, $reason, $quantity);
            }
        }
        $event = \Sentry\Event::createClientReport();
