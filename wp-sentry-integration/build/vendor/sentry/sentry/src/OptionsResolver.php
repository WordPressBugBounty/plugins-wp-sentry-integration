...
class OptionsResolver
{
     ...
     */
    public function resolve(array $options = [], ?\WPSentry\ScopedVendor\Psr\Log\LoggerInterface $logger = null) : array
    {
     ...
     * @return array<string, mixed>
     */
    public function resolveOnly(array $override = [], array $options = [], ?\WPSentry\ScopedVendor\Psr\Log\LoggerInterface $logger = null) : array
    {
    ...
    private function processDefaults(array $defaults, string $parentPath) : array
    {
        ...
        foreach ($defaults as $option => $value) {
            ...
            } elseif (!\Sentry\Util\Arr::isAssociative($processed)) {
     ...
     * @return array<string, mixed>
     */
    private function applyOptions(array $resolved, array $defaults, array $options, string $parentPath, ?\WPSentry\ScopedVendor\Psr\Log\LoggerInterface $logger) : array
    {
        /** @mago-ignore analysis:mixed-assignment */
        foreach ($options as $option => $value) {
            ...
            $isBranch = \Sentry\Util\Arr::isAssociative($default);
            /** @mago-ignore analysis:mixed-assignment */
    ...
    private function validateValue(string $name, $value) : bool
    {
        ...
        if (\is_callable($allowedValue)) {
            return $allowedValue($value) === \true;
        }
        if (!\is_array($allowedValue)) {
            return $value === $allowedValue;
        }
        return \in_array($value, $allowedValue, \true);
    }
}
