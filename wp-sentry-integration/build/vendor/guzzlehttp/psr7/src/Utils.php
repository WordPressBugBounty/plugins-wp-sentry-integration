...
final class Utils
{
     ...
     */
    private static function writeAll(\WPSentry\ScopedVendor\Psr\Http\Message\StreamInterface $dest, string $buf) : bool
    {
        $written = 0;
        $len = \strlen($buf);
        while ($written < $len) {
    ...
    private static function warnOnInvalidModifyRequestChanges(array $changes) : void
    {
        ...
        if (\array_key_exists('uri', $changes) && !$changes['uri'] instanceof \WPSentry\ScopedVendor\Psr\Http\Message\UriInterface) {
    ...
    {
        \WPSentry\ScopedVendor\trigger_deprecation('guzzlehttp/psr7', '2.11', 'Passing %s to Utils::modifyRequest() change "%s" is deprecated; guzzlehttp/psr7 3.0 requires %s.', \get_debug_type($value), $key, $expected);
    }
    /**
    ...
    public static function streamFor($resource = '', array $options = []) : \WPSentry\ScopedVendor\Psr\Http\Message\StreamInterface
    {
        if (\is_scalar($resource)) {
            if (!\is_string($resource)) {
                \WPSentry\ScopedVendor\trigger_deprecation('guzzlehttp/psr7', '2.12', 'Passing %s to Utils::streamFor() is deprecated; cast it to a string. guzzlehttp/psr7 3.0 will only accept string, resource, StreamInterface, Stringable, Iterator, callable, or null.', \gettype($resource));
