...
final class Message
{
    ...
    public static function toString(\WPSentry\ScopedVendor\Psr\Http\Message\MessageInterface $message) : string
    {
        ...
        foreach ($message->getHeaders() as $name => $values) {
            if (\is_string($name) && \WPSentry\ScopedVendor\GuzzleHttp\Psr7\Utils::asciiToLower($name) === 'set-cookie') {
    ...
    public static function parseMessage(string $message) : array
    {
        ...
        if ($count !== \substr_count($rawHeaders, "\n")) {
            ...
            $hasFoldedHeader = \preg_match(\WPSentry\ScopedVendor\GuzzleHttp\Psr7\Rfc7230::HEADER_FOLD_REGEX, $rawHeaders);
    ...
    private static function getHostFromHeaders(array $headers) : ?string
    {
        $hostKey = \array_filter(\array_keys($headers), function ($k) {
            ...
            $k = (string) $k;
            return \WPSentry\ScopedVendor\GuzzleHttp\Psr7\Utils::asciiToLower($k) === 'host';
        });
        if (!$hostKey) {
            return null;
        }
        $host = $headers[\reset($hostKey)][0];
        if (!\is_string($host) || \WPSentry\ScopedVendor\GuzzleHttp\Psr7\Rfc7230::parseHostHeader($host) === null) {
